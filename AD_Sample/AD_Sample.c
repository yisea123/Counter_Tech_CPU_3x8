...
void set_adc1_sample_time (void)
{
	...
	switch (g_counter[0].set_adc_mode)
	{
//		case 0:
...
uint32_t send_IR_value (void)
{
	...
	for (i = 0; i < CHANEL_NUM; i++){
		if (send_ad8804_ch_value (i, g_counter[0].view_IR_DA_value[i]) == 0){
			timeout++;
			if (timeout >= 2){
				return timeout;
			}
	...
	for(i = 0; i < CHANEL_NUM; i++){
		...
		while (value_updated){//每个通道单独调压
			...
			if (After_filter[i] < STD_REF_VALUE_L){
				if (g_counter[0].view_IR_DA_value[i] < 160){
					g_counter[0].view_IR_DA_value[i]++;
					value_updated++;
				}
			}else if (After_filter[i] > STD_REF_VALUE_H){
				if (g_counter[0].view_IR_DA_value[i] > 0){
					g_counter[0].view_IR_DA_value[i]--;
					value_updated++;
				}
			}
...
uint32_t calibrate_IR_1 (void)
{
	...
	while (value_updated > 0){
		...
		for(i = 0; i < CHANEL_NUM; i++){
			if (After_filter[i] < STD_REF_VALUE_L){
				if (g_counter[0].view_IR_DA_value[i] < 255){
					g_counter[0].view_IR_DA_value[i]++;
					value_updated++;
				}
			}else if (After_filter[i] > STD_REF_VALUE_H){
...
void re_calibration_detect (void)
{
	...
	for (i = 0; i < CHANEL_NUM; i++){
		g_counter[0].ch[i].ad_max = 0;
		g_counter[0].ch[i].ad_min = 0xFFFF;
		g_counter[0].ch[i].std_v = 0;
		g_counter[0].ch[i].ad_averaged_value = 0;
		for (j = 0; j < AD_FITTER_BUFF_SIZE; j++){
			g_counter[0].ch[i].ad_fitter_buff[j] = 0;
		}
	}
	process_rdy = 0;
	AD_Sample_init ();
...
uint16_t get_ad_averaged_value (uint16_t C, uint16_t ad_value)
{
	...
	int i;
	g_counter[0].ch[C].ad_fitter_index %= AD_FITTER_BUFF_SIZE; 
	g_counter[0].ch[C].ad_fitter_buff[g_counter[0].ch[C].ad_fitter_index] = ad; 
	index = g_counter[0].ch[C].ad_fitter_index;
	for (i = 0; i < AD_FITTER_BUFF_SIZE; i++){
		index++;
		index %= AD_FITTER_BUFF_SIZE;
		AD[i] = g_counter[0].ch[C].ad_fitter_buff[index];
	}
	g_counter[0].ch[C].ad_fitter_index++; 
	...
	uint16_t ad = ad_value;
	g_counter[0].ch[C].ad_fitter_index %= AD_FITTER_BUFF_SIZE; 
	g_counter[0].ch[C].ad_averaged_value += ad; 
	g_counter[0].ch[C].ad_averaged_value -= g_counter[0].ch[C].ad_fitter_buff[g_counter[0].ch[C].ad_fitter_index]; 
	g_counter[0].ch[C].ad_fitter_buff[g_counter[0].ch[C].ad_fitter_index] = ad; 
	ad = g_counter[0].ch[C].ad_averaged_value / AD_FITTER_BUFF_SIZE; 
	g_counter[0].ch[C].ad_fitter_index++; 
	return ad;
#endif
}
...
{
	g_counter[0].ch[C].std_down_offset = (g_counter[0].ch[C].ad_max - g_counter[0].ch[C].ad_min);
	g_counter[0].ch[C].std_up_offset = (g_counter[0].ch[C].ad_max - g_counter[0].ch[C].ad_min - 1);
	g_counter[0].ch[C].std_down_offset *= g_counter[0].set_std_numerator;
	g_counter[0].ch[C].std_up_offset *= g_counter[0].set_std_numerator;
//	g_counter[0].ch[C].std_down_offset /= (g_counter[0].set_std_denumerator);
//	g_counter[0].ch[C].std_up_offset /= (g_counter[0].set_std_denumerator);	
	g_counter[0].ch[C].std_down_offset /= 10;
	g_counter[0].ch[C].std_up_offset /= 10;
}


//  \
	g_counter[0].ch[C].std_v = AD[C]; \
...
#define AD_PRE_FITTER(AD,BUF,C,S) { \
		...
		if (process_rdy > START_DATA){ \
			g_counter[0].ch[C].ad_averaged_value += AD[C]; \
			g_counter[0].ch[C].ad_averaged_value -= g_counter[0].ch[C].ad_fitter_buff[g_counter[0].ch[C].ad_fitter_index]; \
			g_counter[0].ch[C].ad_fitter_buff[g_counter[0].ch[C].ad_fitter_index] = AD[C]; \
			AD[C] = g_counter[0].ch[C].ad_averaged_value / AD_FITTER_BUFF_SIZE; \
		}else if (process_rdy < START_DATA){ \
			g_counter[0].ch[C].ad_fitter_buff[g_counter[0].ch[C].ad_fitter_index] = AD[C]; \
		}else if (process_rdy == START_DATA){ \
			uint16_t i; \
			g_counter[0].ch[C].ad_fitter_buff[g_counter[0].ch[C].ad_fitter_index] = AD[C]; \
			g_counter[0].ch[C].ad_averaged_value = 0; \
			for (i = 0; i < AD_FITTER_BUFF_SIZE; i++){ \
				g_counter[0].ch[C].ad_averaged_value += g_counter[0].ch[C].ad_fitter_buff[i]; \
			} \
		} \
		g_counter[0].ch[C].ad_fitter_index++; \
	}
...
#define START_DATA 5
	...
	#define GET_STD_AD_V(AD,BUF,C,S)  {\
		...
		if (process_rdy > START_DATA){ \
			if (AD[C] > g_counter[0].ch[C].ad_max){ \
				g_counter[0].ch[C].ad_max = AD[C]; \
			} \
			if (AD[C] < g_counter[0].ch[C].ad_min){ \
				g_counter[0].ch[C].ad_min = AD[C]; \
			} \
			if (process_rdy != process_rdy_old) { \
				g_counter[0].ch[C].std_v += AD[C] / (PROCESS_RDY - (START_DATA ) - 1); \
			}\
		} \
...
//	}
	...
	if (_ch == g_counter[0].set_watch_ch){
...
void DMA1_Channel1_IRQHandler(void)
{
	...
	if ((dma_irq_cycle > 2100) && (process_rdy == PROCESS_RDY)){
		g_counter[0].system_status = ADC_TIME_ERROR;
	}
/////////////////////////////////////////////////////////////////////////////////
//	u8 count;
	if(DMA_GetITStatus(DMA1_IT_HT1)){
		g_counter[0].AD_buf_p = AD_DMA_buf.AD_Value_0;
	}else if(DMA_GetITStatus(DMA1_IT_TC1)){
		g_counter[0].AD_buf_p = AD_DMA_buf.AD_Value_1;
	}else if (DMA_GetITStatus(DMA1_IT_TE1)){
		g_counter[0].AD_buf_p = 0;
	}
	DMA_ClearITPendingBit(DMA1_IT_GL1); //清除全部中断标志
	if (g_counter[0].AD_buf_p != 0){
		if (process_rdy < PROCESS_RDY){
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 0, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 1, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 2, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 3, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 4, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 5, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 6, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 7, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 8, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 9, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 10, SAMPLE_NUM);
			GET_STD_AD_V (After_filter, g_counter[0].AD_buf_p, 11, SAMPLE_NUM);

			if ((process_rdy + 1) == PROCESS_RDY){
				for (i = 0; i < CHANEL_NUM; i++){
					if (g_counter[0].ch[i].std_v > STD_UP_V_OFFSET) {
						g_counter[0].ch[i].std_up_v = g_counter[0].ch[i].std_v - STD_UP_V_OFFSET;
					}else{
						g_counter[0].ch[i].std_up_v = g_counter[0].ch[i].std_v;
					}
					if (g_counter[0].ch[i].std_v > STD_DOWN_V_OFFSET) {
						g_counter[0].ch[i].std_down_v = g_counter[0].ch[i].std_v - STD_DOWN_V_OFFSET;
					}else{
						g_counter[0].ch[i].std_down_v = g_counter[0].ch[i].std_v;
					}
				}
		...
		//////////////////////////////////////////////////////////////////////////////////
			...
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 0, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 1, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 2, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 3, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 4, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 5, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 6, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 7, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 8, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 9, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 10, SAMPLE_NUM);
			AD_FILTER (After_filter, g_counter[0].AD_buf_p, 11, SAMPLE_NUM);

			//After_filter[0] = g_counter[0].sim_ad_value;
			r_code += detect_ad_value_process (&g_counter[0].ch[0], After_filter[0], 0);
			r_code += detect_ad_value_process (&g_counter[0].ch[1], After_filter[1], 1);
			r_code += detect_ad_value_process (&g_counter[0].ch[2], After_filter[2], 2);
			r_code += detect_ad_value_process (&g_counter[0].ch[3], After_filter[3], 3);
			r_code += detect_ad_value_process (&g_counter[0].ch[4], After_filter[4], 4);
			r_code += detect_ad_value_process (&g_counter[0].ch[5], After_filter[5], 5);
			r_code += detect_ad_value_process (&g_counter[0].ch[6], After_filter[6], 6);
			r_code += detect_ad_value_process (&g_counter[0].ch[7], After_filter[7], 7);
			r_code += detect_ad_value_process (&g_counter[0].ch[8], After_filter[8], 8);
			r_code += detect_ad_value_process (&g_counter[0].ch[9], After_filter[9], 9);
			r_code += detect_ad_value_process (&g_counter[0].ch[10], After_filter[10], 10);
			r_code += detect_ad_value_process (&g_counter[0].ch[11], After_filter[11], 11);
			...
			if (my_env.print == 1){
				if (r_code != 0){
				}else if (g_counter[0].ch[g_counter[0].set_watch_ch].state == CH_DATA_RDY){
					if (my_env.dma_state == 0){
						my_env.dma_state = 1;
						g_counter[0].buf_addr = (u32)&Detect_Buf[0];
						OSQPost(debug_msg, (void *) 0x55);//发送消息
					}
				}
			}else if ((AD_buff.buffer_en == 1)){
				AD_buff.buffer[AD_buff.buffer_index] = After_filter[g_counter[0].set_watch_ch];
