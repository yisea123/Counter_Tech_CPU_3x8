...
#define MAP_DOOR_TO_EXT_OUTPUT(C_CH, EX_CH, DOOR_CH){ \
	if (counter_env.system_signal == SYSTER_STOP){ \
		EXT_OUT##EX_CH = GET_BIT (counter_module[C_CH].output_buf_low_manual, DOOR##DOOR_CH); \
	}else{ \
...
typedef struct{
	...
	//uint16_t alarm_low;
	//uint16_t alarm_low_enable;
}s_counter_module;

// 
#define MODBUS_DA_NUM 3
#define VIB_DA_NUM 6
typedef struct{
	uint16_t auto_scan;
	uint16_t clear_monitor_data;
	uint16_t clear_product_data;
...
//	uint16_t screw_is_run;
	...
	uint16_t total_good;
	uint16_t ext_total_good;
	uint16_t system_signal;
	uint16_t screw_shift_bottle_status;
	uint16_t screw_running;
	uint16_t detect_error;
	uint16_t procuct_num_reach;
	uint16_t continue_reject_flag;
	uint16_t sys_config_info;
	uint16_t system_alarm_high;
	uint16_t system_alarm_high_enable;
	uint16_t system_alarm_low;
	uint16_t system_alarm_low_enable;
	uint16_t led_yellow;
	uint16_t led_yellow_flash_ctr;
	uint16_t beep;
	uint16_t beep_flash_ctr;
	uint16_t is_master;
	...
	uint16_t set_product_num;
	uint16_t set_continue_reject_stop_num;
	uint16_t set_pre_open_door_time;
	uint16_t set_count;
	...
	uint16_t counter_da_module_real[MODBUS_DA_NUM];
	uint16_t counter_da_module_read_back[MODBUS_DA_NUM];
	uint16_t counter_da_module_test[MODBUS_DA_NUM];
	uint16_t counter_da_module_apply[MODBUS_DA_NUM];
}s_counter_env;
