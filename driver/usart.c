...
unsigned char uart2_rec_buf[UART2_BUF_LEN];   
void uart2_dma_config (void)
{
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = DMA1_7_INT_PREEM;  
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = DMA1_7_INT_SUB;  
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;  
	NVIC_Init(&NVIC_InitStructure);    
	//---------------------串口功能配置---------------------   
	...
	DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)(&USART2->DR);  
	//内存地址  
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)uart2_rec_buf;  
	//dma传输方向单向  
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;  
	//设置DMA在传输时缓冲区的长度  
	DMA_InitStructure.DMA_BufferSize = UART2_BUF_LEN;  
	//设置DMA的外设递增模式，一个外设  
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;  
	//设置DMA的内存递增模式  
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;  
...
{  
	...
	if(DMA_GetITStatus(DMA1_IT_TC7)){
		...
		//OSQPost(modbusRTU_msg, (void *) 0xA1);//发送消息
...
{        
	//设置传输数据长度  
	IO_RS485_TX_EN=0;//切换为接收模式
	memset (uart2_rec_buf, 0, UART2_BUF_LEN);
	DMA_Cmd(DMA1_Channel6,DISABLE);
	DMA_SetCurrDataCounter(DMA1_Channel6, UART2_BUF_LEN);  
	DMA_Cmd(DMA1_Channel6,ENABLE);
...
//	GPIO_PinRemapConfig(GPIO_Remap_USART2, ENABLE); 
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	//复用推挽输出
	GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化GPIOD5
   
	//USART2_RX	  GPIOD6初始化
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;//
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//浮空输入
	GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化GPIOD6 
...
{     
    ...
    if(USART_GetITStatus(USART2, USART_IT_IDLE) != RESET){  
			...
			temp = temp;
			OSQPost(modbusRTU_msg, (void *) 0xA0);//发送消息	 
    }   
#if SYSTEM_SUPPORT_OS 	//如果SYSTEM_SUPPORT_OS为真，则需要支持OS.
	OSIntExit();  											 
#endif 
